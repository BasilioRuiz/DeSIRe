...
c	Calcula los factores de Lande efectivos
	...
	subroutine landesub(iln,wlengt,factor)
	implicit real*4 (a-h,o-z)
	parameter (mc=20)	!numero maximo de componentes zeeman
	parameter (zeff=5.)	!semiempirico para el damping gamma6
	integer mult(2),ji(2),jf(2)
	real tam(2),G(2),loggf
	real dlp(mc),dll(mc),dlr(mc),sp(mc),sl(mc),sr(mc)
	character design(2)*1	
	character atom*2


	call leelineasii (iln,atom,istage,wlengt,zeff,energy,loggf,mult,
     &                  design,tam)
...
c	e intensidad (sp,sl,sr)

	call SUBLANDE(mc,mult,design,tam,ji,jf,dlo,np,nl,nr,dlp,dll,dlr,
     &	sp,sl,sr,G)
...
c                              r=13/2,t=15/2, u=17/2,v=19/2,w=21/2
      SUBROUTINE sublande(MC,MULT,DESIGN,TAM,JI,JF,DL0,NP,NL,NR,DLP,DLL,
     *DLR,SP,SL,SR,g)
	implicit real*4 (a-h,o-z)
...
6     TAM(I)=1.              ! TOTAL ANGULAR MOMENTUM
      print*,' tras 6 en sublande ',level
      ...
      GO TO 8   
7     CONTINUE  
c      print*,'en sublande: design=',design(i)
      do 70 j=1,21 
c      print*,'en sublande: design(',i,')=',design(i) ,'code1(',j,')=',code1(j) 
...
14    I=2*JI1+1              ! HALF-INTEGRAL J'S
      print*,'0 en sublande otro',level
...
35    MUMIN=-JI2
      ...
      print*,'1 en sublande otro',np,nl,nr
