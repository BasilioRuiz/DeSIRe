...
c departure coefficients	
	real*4 beta1_1(kl,kt), beta2_1(kl,kt)
        real*4 beta1_2(kl,kt), beta2_2(kl,kt)
c comunes
...
c        print*,'fperfil2err 97 atmostry(1) ,atmosr(1)=',atmostry(6*ntau+1),atmosr(1)
	if(nciclos.ne.0)call amp2(ntau,m,atmostry,atmosr)
c        print*,'fperfil2err 99 atmostry(1) ,atmosr(1)=',atmostry(6*ntau+1),atmosr(1)
...
c ************************* atmosfera 1 ***************************************
	...
	if(abs(fill1).gt.epsilon)then
	   if(nohaycampo1.eq.0.and.ist(1).ne.0)then
	      call blendscon2(atmos1,scal1,rt,rp,rv,rm,mnod1,beta1_1,beta2_1)
	      ...
	      call sub1err(m(5),vmac1,fill,rv,k,dscal)      !velocidad 
              ...
              call mult1err(m(8),ntotal4,k,fill,vmac1,sin0,dscal) !macro
       	      ...
       	      call blends2(atmos1,scal1,rt,rp,rh,rv,rg,rf,rm,mnod1,beta1_1,beta2_1)
...
c ************************* atmosfera 2 ***************************************
	...
	if(abs(fill2).gt.epsilon)then
	   if(nohaycampo2.eq.0.and.ist(1).ne.0)then
              call blendscon2(atmos2,scal2,rt,rp,rv,rm,mnod2,beta1_2,beta2_2)
	      ...
	      call sub1err(m(13),vmac2,fill,rv,k,dscal)      !velocidad
              ...
              call mult1err(m(16),ntotal4,k,fill,vmac2,sin0,dscal) !macro
       	      ...
       	      call blends2(atmos2,scal2,rt,rp,rh,rv,rg,rf,rm,mnod2,beta1_2,beta2_2)
c Dado que dlamda0 entra en el common via blends2 la sentencia siguiente 
c no puede colocarse antes de la llamada a blends2  
	      k1=0
	      do i=1,4
