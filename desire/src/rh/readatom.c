/* ------- file: -------------------------- readatom.c --------------
       ...
       Last modified: Fri May  7 17:05:59 2021 --
...
void freeAtomicContinuum(AtomicContinuum *continuum)
{
  ...
  // 06/06/21 epm: Is it save to ignore the following 'if'?
  // Otherwise the memory is not freed (see sortlambda.c::385 & 405).
  // if (!continuum->hydrogenic)
    free(continuum->alpha);
}
