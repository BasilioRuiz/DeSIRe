/* ------- file: -------------------------- piecewise1d.c -----------
       ...
       Last modified: Wed May 23 09:47:14 2018 --
  ...
  
{
  ...
  double dtau_uw, dtau_dw, dS_uw, I_upw, dS_dw, c1, c2, w[3],
         zmu, Bnu[2];
  double dsup,dsdn, dt, dt2, dt3,ex, alpha, beta, alphaprim,
         betaprim, dsup2;
  ...
  k    = k_start + dk;
  dsup = fabs(geometry.height[k] -
	      geometry.height[k-dk]) / geometry.muz[mu];
  dsdn = fabs(geometry.height[k+dk] -
	      geometry.height[k]) / geometry.muz[mu];
  ...
  dtau_uw = 0.5 * dsup * (chi[k-dk]+chi[k])  + 1./12. *
    dsup*dsup * (dchi_up - dchi_c );
  ...
  for (k = k_start+dk;  k != k_end+dk;  k += dk) {
    ...
    if (k != k_end) {
      ...
      /* --- Downwind path length --                   -------------- */
       ...
       dsdn = fabs(geometry.height[k+dk] - geometry.height[k]) /
	 geometry.muz[mu];
	 ...
	 dsdn2=fabs(geometry.height[k+2*dk] -
		    geometry.height[k+dk]) / geometry.muz[mu];
	 har_mean_deriv(&dchi_dn, dsdn, dsdn2, chi[k],chi[k+dk],
			chi[k+2*dk]);       
       } else {
	 dchi_dn = (chi[k + dk] - chi[k]) / dsdn;
       ...
       dtau_dw = 0.5 * dsdn * (chi[k]+chi[k+dk]) + 1./12.* dsdn * dsdn *
	 (dchi_c  - dchi_dn) ;
	...
	betaprim  =   (1.0/30.0)*dt3 - (1.0/12.0)*dt2;
      ...
      I[k]= I_upw*ex + alpha*S[k-dk] + beta*S[k] + alphaprim*dS_up +
	betaprim*dS_c;
