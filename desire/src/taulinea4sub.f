...
c        if(dabs(logcth*1.d0).le.epsilon*1.d-2 .and. il.eq. 0 )return
	...
	if(il.eq.0)then
	   do i=1,ntau
	      tau(i)=ttau(i)-logcth
c             vz(i)=vx*sth+vz(i)*cth
              vzi=-vz(i)               !changing sign vz>0 downward
...
c	      vy2= vy1
	      vz2=-vx1*slon+vz1*clon
	      
c	      vz(i)=vx*clat*slon+vz(i)*cth !changing sign vz>0 downward vx>0 positive X, right of the disk  
              vz(i)=-vz2   !changing sign vz>0 downward vx>0 positive X, right of the disk 

c evaluating new gamma and phi
              cga=Bz2      
c	      cga=-sth*cfiz*sgaz+cth*cgaz
	      if(cga.gt.1.)then
                  print*,'cos(gamma) larger than 1'
		  cga=1.-epsilon/100.
	      endif
	      if(cga.lt.-1.)then
                  print*,'cos(gamma) lower than -1'
		  cga=-1.+epsilon/100.
	      endif
...
c	       vz(i)=(vz(i)-vx*sth)/cth
               vzi=vz(i)               !keep the sign because I'm resolving vz
...
c	       vz(i)=(vz(i)-vx*clat*slon)/cth
	       vz(i)=(vz(i)-vx*slon-vy*slat*clon)/cth	       
c evaluating new gamma and phi
               cgaz=Bz2      
c               if(i.eq.1)print*,'taulinea4sub 153 cgaz=',cgaz
...
c	          gamma(i)=atan2(sgaz,cgaz)*180./pi
c                  gamma(i)=acos(cgaz)/pi180    !in degrees (LOS reference system)
                  cfiz=Bx2/sgaz
	          sfiz=By2/sgaz                
c	          sfiz=sfi*sga/sgaz
c	          cfiz=(cth*cfi*sga-sth*cga)/sgaz
	          phi(i)=atan2(sfiz,cfiz)/pi180 !in degrees (LOS reference system)
               end if
c                  if(i.eq.1)print*,'taulinea4sub 164',gamma(i),cgaz,clon,slon,clat,slat
