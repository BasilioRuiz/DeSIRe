...
c departure coefficients
        ...
        real*4 beta1_1(kl,kt), beta2_1(kl,kt)   !,pe_dep1(kt)
        real*4 beta1_2(kl,kt), beta2_2(kl,kt)   !,pe_dep2(kt)
        character*2 atom_nlte(kl),atom
...
c comunes
	common/responde2/ist,ntau,ntl,nlin,npas,nble
        ...
        common/atmosferaoutold/atmosoutold 
	...
	common/zetas/pg1,z1,ro1,pg2,z2,ro2    !para el calculo de z,pg y ro en amp22 y amp22err
        ...
        common/atmos1LGold/atmos1LGold
...
c        print*,'nlam_LTE=',nlam_LTE,'nRFpoints=',nRFpoints

c	ivezfperfil=ivezfperfil+1
c	print*,'fperfil2 103 iprimeravez=',iprimeravez,iRH1,'idiff=',idiff,' &&&&&&&&$$'
	icallingRH1=iRH1
        icallingRH2=iRH2
...
c	if(ivezfperfil.eq.1)then
	  ...
	  ikk0=0
	  iii=0
c	  print*,'fperfil2 116 ntl=',ntl,nlin(1),nlin(2)
	  do iln=1,ntl
	     do ible=1,nble(iln)
		 ixx=ixx+1
                 nxx=nlin(ixx) 
     ...
     &                         loggf,mult,design,tam,alfa,sigma,nlow,nup)
                 ...
                 atom_nlte(ixx)="  "
                 linea_nlte(ixx)=0
                 if(nlow .ne. 0 .or. nup .ne. 0)linea_nlte(ixx)=ixx
                 if(nlow .ne. 0 .or. nup .ne. 0)atom_nlte(ixx)=atom
                 do j=1,ntau
	            beta1_1(ixx,j)=1.0
	            beta2_1(ixx,j)=1.0
	            beta1_2(ixx,j)=1.0
	            beta2_2(ixx,j)=1.0	            
                 ...
                 sigma_arr(ixx)=sigma
                 wave_arr(ixx)=wlengt
c                 print*,'en blends2.f 2 atom nlow nup alfa sigma',atom,nlow,nup,alfa,sigma
...
c                        print*,'fperfil2 129 ',iln,' ikk dlamda =',ikk,dlamda(ikk),' wcon=',wcon,con_i1NLTE(iln)
                        wcon_i1(iln)=wcon
                    end if
                    if(ible.eq.nble(iln) .and. ipas.eq.npas(iln))then
                        wcon=wlengt1+dlamda(ikk)*1.e-3
                        con_i2NLTE(iln)=real(conhsra_RHtable(wcon))    !conhsra(wcon)*real(conhsra_RH(wcon))
c                        print*,'fperfil2 135 ',iln,' ikk dlamda =',ikk,dlamda(ikk),' wcon=',wcon,con_i2NLTE(iln)
                        wcon_i2(iln)=wcon
                    end if
                 end do  
              end do   
...
c                    if(iln .eq. 3)print*,'fperfil2 144',iii,dlamda(iii),continuohNLTEarr(iii)
...
c	print*,'fperfil2 206 atmostry(g1),atmosr(1) =',atmostry(6*ntau+1),atmosr(1)
c        print*,'fperfil2 1 =',atmostry(2),atmostry(ntau+2),atmostry(2*ntau+2),atmostry(3*ntau+2)
	if(nciclos.ne.0.and.iprimeravez.ne.1)call amp2(ntau,m,atmostry,atmosr)
c	print*,'fperfil2 2 =',atmostry(2),atmostry(ntau+2),atmostry(2*ntau+2),atmostry(3*ntau+2)

c	print*,'fperfil2 209 atmostry(g1),atmosr(1)=',atmostry(6*ntau+1),atmosr(1)
...
c	   print*,'z1(ntau) en 2 =',z1(ntau),pg1(ntau),ro1(ntau) 
...
c	   print*,'perfil2 282 IRH1=',atmos1LG(9*ntau+2+i),atmos1LG(10*ntau+2+i)
	end do  
c	stop
...
c ************************* atmosfera 1 ***************************************
	...
	if(abs(fill1).gt.epsilon)then
     ...
     +ntau,ntotal_lines,linea_nlte,atom_nlte,nlow_i,nup_i,beta1_1,beta2_1,stok_RH_1)
...
c	      print*,'fperfil2 icallingRH1=',icallingRH1
c	      print*,'fperfil2 numerical(1)=',numerical(1)
c	      print*,'fperfil2 iprimeravez=',iprimeravez
c	      print*,'fperfil2 iRH1 =',iRH1
c              print*,'fperfil2 icallingRH1numericalT1=',icallingRH1numericalT1,'!!!!!!!!!!!!!!!!!!!!!'
	      if(icallingRH1numericalT1.eq.1)then
c	          print*,'fperfil: evaluating RF to T numerically (no Stokes case)'
...
c                   print*,'nRFpoints=',nRFpoints
                  do i=1,nRFpoints
c                     print*,'RF_LTE=',rt1(i),'RF_RH=',rt1_RH(i)
                     rt1_LTE(i)=rt1(i)
                     rt1(i)=rt1_RH(i)
                  end do                
	      endif
	      if(icallingRH1numericalT1.eq.0 .and. iprimeravez.gt.1 )then
	          do i=1,nRFpoints
c	             RF_LTEold=rt1_LTE(i)
	             delta=rt1(i)-rt1_LTE(i)
                     aa=rt1_RH(i)+delta
                     rt1_LTE(i)=rt1(i)  !storing the new LTE
c                     print*,'RF_LTEnew=',rt1_LTE(i),'RF_LTEold=',RF_LTEold,'RF_RH=',rt1_RH(i),'adopted=',aa
                     rt1(i)=aa          !updating
c                     rt1(i)=rt1_LTE(i)
c                     rt1(i)=rt1_RH(i)
                  end do
...
c   	      print*,'fperfil2  pe(301)=',atmos1(2*ntau+301)

c   	      if(iRH1.eq.1)then 
   	      if(icallingRH1.eq.1)then 
c	         print*,'perfil2 350 IRH1=',iRH1
	         call departures(label_ID_model,RH_model,RH_magneticfield,1,atmos1LG,
     +ntau,ntotal_lines,linea_nlte,atom_nlte,nlow_i,nup_i,beta1_1,beta2_1,stok_RH_1)    !,pe_dep1)
c                 do i=1,ntau
c                    atmos1(2*ntau+i)=atmosnew(2*ntau+i) !copying new pe
c                 end do  
c              else  !if we are NOT calling RH
c                 do i=1,ntau
c                   pg1_change(i)=pg(i)/atmosLG(9*ntau+2+i)
c                    atmos1(2*ntau+i)=atmos1(2*ntau+i)*pe1_change(i)   !copying new pe
c                 end do  
	      end if   
c              print*,'fperfil2 2 pe(301)=',atmos1(2*ntau+301)
...
c              if(iRH1.eq.1)then 
              if(icallingRH1.eq.1)then 
c                 print*,'fperfil2 400 ',dlamda0s(1),dlamda0s(k1),dlamdas(1434),dlamdas(1435),dlamdas(1436)
                 if(vmac1.gt.0 .or. ifiltro.ge.1)then
c                    print*,'fperfil2 402',stok_RH_1(1),stok_RH_1(101),stok_RH_1(1435),stok_RH_1(1436)
	            call deconv(stok_RH_1,1,ntls,npass,dlamda0s,dlamdas,vmac1)
c	            print*,'fperfil2 404',stok_RH_1(1),stok_RH_1(101),stok_RH_1(1435),stok_RH_1(1436)
                 end if 
...
c	      if(iRH2.eq.1)then 
     ...
     +ntau,ntotal_lines,linea_nlte,atom_nlte,nlow_i,nup_i,beta1_2,beta2_2,stok_RH_2)   
c                 do i=1,ntau
c                    atmos2(2*ntau+i)=atmosnew(10*ntau+2+i)  !copying new pe
c                 end do 
...
c                 do i=1,ntau
c                   pg1_change(i)=pg(i)/atmosLG(9*ntau+2+i)
c                    atmos2(2*ntau+i)=atmos2(2*ntau+i)*pe2_change(i)   !copying new pe
c                 end do  
	      end if  	   
...
c        print*,'fperfile2 ntotal4',ntotal4
        do j=1,ntotal4
...
c             print*,'fperfil2 437 ',scal(i),scal_RH(i)
c             if(i.le. 3)print*,'fperfil2 437 ',scal(i),scal_RH(i)
         end do
c         print*,'fperfil2 519 ',scal(1),scal_RH(1),stok_RH_1(1),diff_NLTE_LTE(1)
      end if
c      if(ist(1).eq.0)then
c        print*,'fperfil2 : Stokes I should be synthesized/inverted'
c        stop
c      endif   
c      ii=0
c      
...
c	             print*,'fperfil2 503 ',i,ii,scal(ii),scal_RH(ii),ImasVfactor(ii)
c	          end do   
c	       end do
c            endif
c            if(i.gt.1)then
...
c	             print*,'fperfil2 510 ',i,ii,scal(ii),scal_RH(ii),ImasVfactor(ii)
c	          end do   
c	       end do
c            endif           
c         endif
c      end do
c      print*,'fperfil2 : ntotal4=',ntotal4,ii
...
c          print*,'fperfil2 559 ',scal_corregida(1)
...
c              print*,'fperfil2 586 ',mnod1tot(6),mprimera(6),k
...
c	atomic Number returns the atomic number corresponding to a given symbol
     ...
     *'fr','ra','ac','th','pa','u'/
     ...
     &   .and. symbol.ne.ATM2(iel) .and. symbol.ne.ATM3(iel) )
       ...
       atomic_number=iel
c       print*,'The atomic number of ',symbol,' is ',atomic_number
