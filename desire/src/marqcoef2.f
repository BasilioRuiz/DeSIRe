...
c        print*,'en marqcoef2 T(1:2)=',a(1),a(2)
...
c      if( iRH1 .eq. 1 .or.  iRH2 .eq. 1 .or. ileoNLTE .eq. 1)then
       if( iRH1 .eq. 1 .or.  iRH2 .eq. 1 )then
         idiff=1
         do i=1,ndata
c             if(i.le.3)print*,'marqcoef2 123',scal_RH(npos(i)),scal_RH(i) 
             ymodobs_RH(i)=scal_RH(npos(i))  !synthetic profile with RH
             diff_NLTE_LTE(i)=(ymodobs_RH(i)-ymodobs(i))
c             ynew(i)=y(i)-ymodobs_RH(i)+ymodobs(i) ! y= obseved profile 
c             if(i.lt.5)print*,'marqcoef2 i SIR RH OBS Obsnew',i, ymodobs(i),ymodobs_RH(i),y(i),ynew(i)
         end do
      end if
      
c      print*,'marqcoef2.f idiff=',idiff,'Iobs=',y(243),'ISIR=',ymodobs(243),'IRH=',ymodobs_RH(243),
c     & 'Icorr=',ymodobs(243)+diff_NLTE_LTE(243)
...
cc            ymod(npos(i))=ymodobs(i)
         end do
c         print*,'marqcoef 133 NLTE',scal_RH(npos(1)),' LTE=',ymod(npos(1)),' Final=',ymodobs(1)
      end if
      
c      print*,'marqcoef2 entro en el do con  numberfrec(1)=',numberfrec(1),'mfit=',mfit
...
c         print*,i,y(i),dy,ymodobs_RH(i),ymodobs(i)-diff_NLTE_LTE(i),diff_NLTE_LTE(i)

c        dy=ynew(i)-stokesratio(i)
...
c	print*,'marqcoef2 184',chisq,chireal,sumsq
